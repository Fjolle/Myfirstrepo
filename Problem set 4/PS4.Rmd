...
```{r, warning=FALSE, echo=FALSE}
grocery = read.csv("https://raw.githubusercontent.com/jgscott/ECO395M/master/data/groceries.txt", header = FALSE)

grocery$ID = seq.int(nrow(grocery))
grocery <- cbind(grocery[,5], stack(lapply(grocery[,1:4], as.character)))[1:2]
...
grocery_list = split(x=grocery$items, f=grocery$ID)
...
```{r, warning=FALSE, echo=FALSE}
inspect(subset(rule, confidence > 0.2))
```

```{r, echo=FALSE}
inspect(subset(rule, support > 0.4))
```
...
```{r, warning=FALSE, echo=FALSE}
inspect(subset(rule, lift > 3))
```

Finding the 10“best” associations.

```{r, warning=FALSE, echo=FALSE}
inspect(subset(rule, lift > 3, confidence > 0.2, support > 0.4))
```

```{r, warning=FALSE, echo=FALSE}
inspect(subset(rule, confidence > 0.6))
subset1 = subset(rule, subset=confidence > 0.01 & support > 0.005)
summary(subset1)
```
...
```{r, warning=FALSE, echo=FALSE}
plot(head(subset1, 120, by='lift'), method='graph')
```
